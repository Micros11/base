...
namespace Sungero.RecordManagement.Server
{
  partial class ActionItemExecutionAssignmentFunctions
  {
    ...
    {
      // Проверить наличие невыполненных подчиненных поручений у ведущего задания.
      var subActionItems = Functions.ActionItemExecutionTask.GetSubActionItemExecutions(_obj).ToList();
      if (subActionItems.Any(x => x.IsCompoundActionItem != true))
        return false;
      
      foreach (var actionItem in subActionItems)
